...
for opt_k_carbonic in range(1, 19):
    opt_total_borate = 1
    alkalinity, alkalinity_std = t93.get_alkalinity(opt_k_carbonic, opt_total_borate)
    ...
    zero_lnpCO2_poly = (
        np.log(t93.pCO2)
        - 0.0433 * t93.temperature
        + 0.5 * 8.7e-5 * t93.temperature**2
    )
    v_lnpCO2_poly = (
    ...
    style_pyco2 = dict(
        c="xkcd:strawberry",
        label="PyCO2SYS ({})".format(pwtools.okc_codes[opt_k_carbonic]),
        alpha=0.8,
        lw=2,
        ls=":",
        zorder=3,
    ...
    ax.plot(
        v_temperature,
        np.exp(v_lnpCO2_PyCO2SYS) - f_linear,
        **style_pyco2,
    )
    ...
    ax.legend(
        loc="upper center", bbox_to_anchor=(0.5, -0.3), edgecolor="xkcd:dark", ncol=2
    )
