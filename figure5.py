...
results_calc12 = pyco2.sys(
    par1=soda_monthly.talk.mean("month").data,
    par1_type=1,
    par2=soda_monthly.dic.mean("month").data,
    par2_type=2,
    salinity=soda_monthly.salinity.mean("month").data,
    temperature=soda_monthly.temperature.mean("month").data,
    temperature_out=soda_monthly.temperature.mean("month").data + dt,
    opt_k_carbonic=10,
    opt_total_borate=1,
)
soda_monthly["fCO2_dt_calc12"] = (
    ...
    - results_calc12["fCO2"]
    + soda_monthly.fCO2.mean("month").data,
)

# Convert with the various upsilon options
opt_at = [5, 1, 2, 6]
results_dt = pyco2.sys(
    par1=soda_monthly.fCO2.mean("month").data,
    par1_type=5,
    salinity=soda_monthly.salinity.mean("month").data,
    temperature=soda_monthly.temperature.mean("month").data,
    temperature_out=soda_monthly.temperature.mean("month").data + dt,
    opt_k_carbonic=10,
    opt_total_borate=1,
    opt_adjust_temperature=np.reshape(np.array([[opt_at]]), [len(opt_at), 1, 1]),
    opt_which_fCO2_insitu=1,
)
for i, at in enumerate(opt_at):
    soda_monthly["fCO2_dt_{}".format(at)] = (("lat", "lon"), results_dt["fCO2_out"][i])
...
for i, at in enumerate(opt_at[:2]):
    ...
    fm = soda_monthly["fCO2_dt_{}_diff".format(at)].plot(
        ax=ax,
        vmin=-1.5,
        vmax=1.5,
        cmap="RdBu_r",
