...
lr = linregress(temperature, np.log(pCO2))  # slope comes out as 42.2 not 42.3!
lr_slope = 42.3e-3
lr_intercept = np.mean(np.log(pCO2) - temperature * lr_slope)
pf = np.polyfit(temperature, np.log(pCO2), 2)

fig, ax = plt.subplots(dpi=300)
ax.scatter(
    temperature, pCO2 - np.exp(np.polyval(pf, temperature)), label="Revised quadratic"
)
ax.scatter(
    temperature,
    pCO2 - np.exp(np.polyval([-8.7e-5 / 2, 43.3e-3, 5.49881], temperature)),
    label="Ta93 quadratic",
)
ax.axhline(0, c="k", lw=0.8)
ax.set_xlabel("Temperature / °C")
ax.set_ylabel("Residual $p$CO$_2$ / µatm")
ax.legend()
fig.tight_layout()
...
for opt_k_carbonic in range(1, 19):
    ...
    zero_lnpCO2_poly = (
        np.log(pCO2) - 0.0433 * temperature + 0.5 * 8.7e-5 * temperature**2
    )
    v_lnpCO2_poly = (
    ...
    ax.scatter(
        temperature,
        # np.log(pCO2),
        pCO2 - np.exp(temperature * lr_slope + lr_intercept),
        c="k",
        label="Ta93 measured",
        s=90,
        zorder=0,
    ...
    ax.plot(
        v_temperature,
        np.exp(v_lnpCO2_poly) - np.exp(v_temperature * lr_slope + lr_intercept),
        **style_poly,
    )
    ax.plot(
        v_temperature,
        np.exp(v_lnpCO2_PyCO2SYS) - np.exp(v_temperature * lr_slope + lr_intercept),
        **style_pyco2,
    )
    ax.legend()
    ax.set_xlabel("Equilibration temperature / °C")
    ax.set_ylabel("($p$CO$_2$ – linear fit) / µatm")
    ...
    fig.tight_layout()
    fig.savefig("figures/f01/f01b_{:02.0f}.png".format(opt_k_carbonic))
    plt.show()
    plt.close()
